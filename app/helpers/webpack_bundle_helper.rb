...
module WebpackBundleHelper
  ...
  private
    ...
    def asset_server
      ...
      return "http://#{request.host}:8080/"
    end

    def pro_manifest
      File.read('public/bundle/manifest.json')
    end

    def dev_manifest
      # webpack-dev-serverから直接取得する
      OpenURI.open_uri("#{asset_server}bundle/manifest.json").read
    end

    def test_manifest
      File.read('public/bundle-test/manifest.json')
    end
    ...
    def asset_bundle_path(entry, **options)
      valid_entry?(entry)
      asset_path("#{asset_server}bundle/" + manifest.fetch(entry), **options)
    end
end
